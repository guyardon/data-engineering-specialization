...
d.arr("arrow-main", ARROW_X, ARROW_Y, [[0, 0], [ARROW_W, 0]],
      GRAY[0])
...
for i, (px, label, color) in enumerate(zip(pill_positions, pill_labels, pill_colors)):
    box_id = f"pill-{i}"
    txt_id = f"pill-t-{i}"
    d.rect(box_id, px, PILL_Y, PILL_W, PILL_H, color[0], color[1],
...
for i, (px, label) in enumerate(zip(pill_positions, freq_labels)):
    d.txt(f"freq-{i}", px, LABEL_Y, PILL_W, LABEL_H,
          label, 20, color=GRAY[0])

# Data type labels
for i, (px, label) in enumerate(zip(pill_positions, data_labels)):
    d.txt(f"data-{i}", px, DATA_LABEL_Y, PILL_W, DATA_LABEL_H,
          label, 17, color=pill_colors[i][0])

# === WRITE FILE ===
outfile = "diagrams/ingestion-continuum.excalidraw"
d.save(outfile)
