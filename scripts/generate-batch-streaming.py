...
for i in range(len(etl_steps) - 1):
    ...
    varrow(f"etl-a{i}", etl_steps[i][0], sx, sy, sw, sh,
           etl_steps[i + 1][0], dx, dy, dw, dh)
...
for i in range(len(elt_steps) - 1):
    ...
    varrow(f"elt-a{i}", elt_steps[i][0], sx, sy, sw, sh,
           elt_steps[i + 1][0], dx, dy, dw, dh)
...
for i in range(len(str_steps) - 1):
    ...
    varrow(f"str-a{i}", str_steps[i][0], sx, sy, sw, sh,
           str_steps[i + 1][0], dx, dy, dw, dh)
...
box("str-analytics", "Real-time\nAnalytics", pair_x, side_y, SIDE_W, BOX_H, GREEN_S, GREEN_B)
box("str-ml", "Machine\nLearning", pair_x + SIDE_W + SIDE_GAP, side_y, SIDE_W, BOX_H, GREEN_S, GREEN_B)
...
sb_cons = {"elementId": "str-cons", "focus": 0.3, "gap": ARR_GAP, "fixedPoint": [0.35, 1]}
eb_ana = {"elementId": "str-analytics", "focus": 0, "gap": ARR_GAP, "fixedPoint": [0.5, 0]}
arr("str-a-ana", cons_cx - 15, cons_bottom,
    [[0, 0], [analytics_cx - (cons_cx - 15), analytics_top - cons_bottom]],
    ARROW_COLOR, sb=sb_cons, eb=eb_ana)

# Arrow to ML (down-right)
ml_cx = pair_x + SIDE_W + SIDE_GAP + SIDE_W // 2
ml_top = side_y - ARR_GAP

sb_cons2 = {"elementId": "str-cons", "focus": -0.3, "gap": ARR_GAP, "fixedPoint": [0.65, 1]}
eb_ml = {"elementId": "str-ml", "focus": 0, "gap": ARR_GAP, "fixedPoint": [0.5, 0]}
arr("str-a-ml", cons_cx + 15, cons_bottom,
    [[0, 0], [ml_cx - (cons_cx + 15), ml_top - cons_bottom]],
    ARROW_COLOR, sb=sb_cons2, eb=eb_ml)
...
rect("note-box", note_x, note_y, NOTE_W, NOTE_H, GRAY_S, "#f8f9fa",
     dashed=True, opacity=80, bnd=[{"id": "note-t", "type": "text"}])
txt("note-t", note_x, note_y, NOTE_W, NOTE_H,
    '"Batch is a special case of streaming"', 20, color="#495057", cid="note-box", op=80)
