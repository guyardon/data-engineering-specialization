...
# Main title
d.txt("title", PAD_X, TITLE_Y, CONTENT_W, TITLE_H, "Choosing Ingestion Tools", TITLE_FSZ)
d.txt("subtitle", PAD_X, SUBTITLE_Y, CONTENT_W, SUBTITLE_H, "Key evaluation criteria", SUBTITLE_FSZ, color=GRAY[0])

# --- Container 1: Data Characteristics ---
d.rect("c1", PAD_X, C1_Y, CONTENT_W, C1_H, *BLUE, opacity=30, dashed=True)
# Container header
d.txt("c1-hdr", PAD_X, C1_Y + 8, CONTENT_W, math.ceil(24 * 1.25), "Data Characteristics", 24, color=BLUE[0])
...
for i, (label, color) in enumerate(zip(C1_PILLS, C1_COLORS)):
    ...
    py = pill_start_y
    pid = f"c1-p{i}"
    d.rect(pid, px, py, C1_PILL_W, PILL_H, *color, bnd=[{"id": f"{pid}-t", "type": "text"}])
...
d.txt("c2-hdr", PAD_X, C2_Y + 8, CONTENT_W, math.ceil(24 * 1.25), "Reliability & Durability", 24, color=RED[0])
...
for i, (label, color) in enumerate(zip(C2_PILLS, C2_COLORS)):
    ...
    py = pill_start_y2
    pid = f"c2-p{i}"
    d.rect(pid, px, py, C2_PILL_W, PILL_H, *color, bnd=[{"id": f"{pid}-t", "type": "text"}])
...
# === WRITE ===
outfile = "diagrams/ingestion-considerations.excalidraw"
d.save(outfile)
