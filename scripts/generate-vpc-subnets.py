...
rect("title_r", PAD_X, TITLE_Y, CONTENT_W, title_rect_h, "transparent", "transparent",
     fill="solid", opacity=0, bnd=[{"id": "title_t", "type": "text"}])
txt("title_t", PAD_X, TITLE_Y, CONTENT_W, title_rect_h,
    "Configure VPC and Subnets", 32, cid="title_r")

# --- VPC container (PURPLE, dashed) ---
rect("vpc", VPC_X, VPC_Y, VPC_W, VPC_H, *PURPLE, dashed=True,
     bnd=[{"id": "vpc_t", "type": "text"}])
txt("vpc_t", VPC_X, VPC_Y, VPC_W, VPC_TITLE_H + VPC_PAD,
    "VPC (10.0.0.0/16)", 24, color=PURPLE[0], cid="vpc")

# VPC subtitle
vpc_sub_y = VPC_Y + VPC_PAD + VPC_TITLE_H + 2
txt("vpc_sub", VPC_X, vpc_sub_y, VPC_W, VPC_SUBTITLE_H,
    "Private IP range: 10.0.x.x", 17, color=PURPLE[0])
...
txt("az1_label", az1_x, AZ_Y + 8, AZ_INNER_W, AZ_LABEL_H,
    "Availability Zone 1", 22, color=BLUE[0])
...
txt("pub1_t", pub1_x, pub1_y, SUBNET_W, SUBNET_TITLE_H + 15,
    "Public Subnet", 22, cid="pub1")
txt("pub1_cidr", pub1_x, pub1_y + SUBNET_TITLE_H + 15, SUBNET_W, SUBNET_CIDR_H,
    "10.0.1.0/24", 17, color=YELLOW[0])
...
txt("priv1_t", pub1_x, priv1_y, SUBNET_W, SUBNET_TITLE_H + 15,
    "Private Subnet", 22, cid="priv1")
txt("priv1_cidr", pub1_x, priv1_y + SUBNET_TITLE_H + 15, SUBNET_W, SUBNET_CIDR_H,
    "10.0.2.0/24", 17, color=RED[0])
...
txt("az2_label", az2_x, AZ_Y + 8, AZ_INNER_W, AZ_LABEL_H,
    "Availability Zone 2", 22, color=BLUE[0])

# Public subnet 2 (YELLOW dashed)
pub2_x = az2_x + AZ_PAD
rect("pub2", pub2_x, pub1_y, SUBNET_W, SUBNET_H, *YELLOW, dashed=True,
     bnd=[{"id": "pub2_t", "type": "text"}])
txt("pub2_t", pub2_x, pub1_y, SUBNET_W, SUBNET_TITLE_H + 15,
    "Public Subnet", 22, cid="pub2")
txt("pub2_cidr", pub2_x, pub1_y + SUBNET_TITLE_H + 15, SUBNET_W, SUBNET_CIDR_H,
    "10.0.3.0/24", 17, color=YELLOW[0])

# Private subnet 2 (RED dashed)
rect("priv2", pub2_x, priv1_y, SUBNET_W, SUBNET_H, *RED, dashed=True,
     bnd=[{"id": "priv2_t", "type": "text"}])
txt("priv2_t", pub2_x, priv1_y, SUBNET_W, SUBNET_TITLE_H + 15,
    "Private Subnet", 22, cid="priv2")
txt("priv2_cidr", pub2_x, priv1_y + SUBNET_TITLE_H + 15, SUBNET_W, SUBNET_CIDR_H,
    "10.0.4.0/24", 17, color=RED[0])
...
for i, (label, color) in enumerate(pill_items):
    ...
    rid = f"pill_{i}"
    tid = f"pill_t_{i}"
    rect(rid, px, PILL_Y, PILL_W, PILL_H, *color,
...
# === WRITE FILE ===
import os
outdir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "diagrams")
