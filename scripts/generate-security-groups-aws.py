...
def gen(dark: bool):
    ...
    graph_attr = {
        "bgcolor": bg, "fontcolor": fc, "fontsize": "32",
        "fontname": "Helvetica Bold", "pad": "0.3",
        "nodesep": "3.0", "ranksep": "0.7", "dpi": "150",
        "label": "Security Groups\n\n", "labelloc": "t",
        "rankdir": "TB", "compound": "true",
    }
    node_attr = {"fontsize": "20", "fontname": "Helvetica", "fontcolor": fc, "height": "1.7"}
    edge_attr = {"color": edge_color, "penwidth": "3.0"}

    def cattr(key):
        c = cc[key]
        return {
            "fontsize": "22", "fontname": "Helvetica Bold", "penwidth": "1.5",
            "labeljust": "c", "labelloc": "t", "style": "dashed,rounded",
            "margin": "18", "bgcolor": c["bg"], "fontcolor": c["fc"], "pencolor": c["border"],
        }

    out_path = os.path.join(OUT_DIR, f"{DIAGRAM_NAME}{suffix}")
    png_path = out_path + ".png"

    with Diagram("", filename=out_path, show=False, direction="TB",
                 graph_attr=graph_attr, node_attr=node_attr, edge_attr=edge_attr, outformat="png") as d:
        user = User("Internet\n(0.0.0.0/0)")
        e = lambda **kw: Edge(color=edge_color, **kw)

        with Cluster("VPC", graph_attr=cattr("vpc")):
            with Cluster("ALB Security Group\nInbound: HTTP/HTTPS from 0.0.0.0/0", graph_attr=cattr("alb_sg")):
                alb = ALB("ALB")

            with Cluster("EC2 Security Group\nInbound: from ALB SG", graph_attr=cattr("ec2_sg")):
                ec2 = EC2("EC2")
        ...
        user >> e(lhead="cluster_VPC", label="HTTP/HTTPS\n(80, 443)", fontsize="16", fontname="Helvetica", fontcolor=fc) >> alb
        alb >> e(label="Source: ALB SG", fontsize="16", fontname="Helvetica", fontcolor=fc) >> ec2
        ec2 >> e(label="Source: EC2 SG", fontsize="16", fontname="Helvetica", fontcolor=fc) >> rds
