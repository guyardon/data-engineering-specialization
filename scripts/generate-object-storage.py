...
rect("obj_hdr", OBJ_HDR_X, OBJ_HEADER_Y, OBJ_HDR_W, OBJ_HDR_H,
     BLUE[0], BLUE[1], bnd=[{"type": "text", "id": "obj_hdr_txt"}])
txt("obj_hdr_txt", OBJ_HDR_X, OBJ_HEADER_Y, OBJ_HDR_W, OBJ_HDR_H,
    "Object", 24, cid="obj_hdr")
...
arr("obj_arrow", ARROW_X, ARROW_START_Y + ARROW_GAP // 2,
    [[0, 0], [0, ARROW_LEN]], BLUE[0])
...
for i, comp in enumerate(components):
    ...
    label = f"{comp['title']}\n{comp['sub']}"
    rect(cid, px, COMP_Y, PILL_W, PILL_H,
         BLUE[0], BLUE[1], bnd=[{"type": "text", "id": tid}])
...
txt("adv_label", PAD_X, ADV_LABEL_Y, CONTENT_W, ADV_LABEL_H,
    "Why Object Storage?", 24, color=GREEN[0])
...
for i, adv in enumerate(advantages):
    ...
    label = f"{adv['title']}\n{adv['sub']}"
    rect(aid, gx, gy, GRID_PILL_W, GRID_PILL_H,
         GREEN[0], GREEN[1], bnd=[{"type": "text", "id": atid}])
