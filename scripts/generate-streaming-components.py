...
txt("title", PAD_X, TITLE_Y, CONTENT_W, TITLE_H,
    "Streaming Architecture", TITLE_FSZ)

# --- Producer ---
rect("prod", BOX_X, Y1, BOX_W, BOX_H, *GREEN,
     bnd=[{"id": "prod-t", "type": "text"}])
txt("prod-t", BOX_X, Y1 + top_pad, BOX_W, title_h,
    "Producers", HDR_TITLE_FSZ, cid="prod")
txt("prod-sub", BOX_X, Y1 + top_pad + title_h + gap_ts, BOX_W, sub_h,
    "Generate events", HDR_SUB_FSZ, color=GREEN[0])

# --- Event Collector ---
rect("coll", BOX_X, Y2, BOX_W, BOX_H, *BLUE,
     bnd=[{"id": "coll-t", "type": "text"}])
txt("coll-t", BOX_X, Y2 + top_pad, BOX_W, title_h,
    "Event Collector", HDR_TITLE_FSZ, cid="coll")
txt("coll-sub", BOX_X, Y2 + top_pad + title_h + gap_ts, BOX_W, sub_h,
    "Groups into batches", HDR_SUB_FSZ, color=BLUE[0])

# --- Broker / Router ---
rect("broker", BOX_X, Y3, BOX_W, BOX_H, *YELLOW,
     bnd=[{"id": "broker-t", "type": "text"}])
txt("broker-t", BOX_X, Y3 + top_pad, BOX_W, title_h,
    "Broker / Router", HDR_TITLE_FSZ, cid="broker")
txt("broker-sub", BOX_X, Y3 + top_pad + title_h + gap_ts, BOX_W, sub_h,
    "Routes messages", HDR_SUB_FSZ, color=YELLOW[0])

# --- Fan-out: Message Queue & Streaming Platform ---
rect("mq", PAIR_X1, Y4, PAIR_W, BOX_H, *PURPLE,
     bnd=[{"id": "mq-t", "type": "text"}])
txt("mq-t", PAIR_X1, Y4 + top_pad, PAIR_W, title_h,
    "Message Queue", HDR_TITLE_FSZ, cid="mq")
txt("mq-sub", PAIR_X1, Y4 + top_pad + title_h + gap_ts, PAIR_W, sub_h,
    "Buffers (SQS, FIFO)", HDR_SUB_FSZ, color=PURPLE[0])

rect("plat", PAIR_X2, Y4, PAIR_W, BOX_H, *CYAN,
     bnd=[{"id": "plat-t", "type": "text"}])
txt("plat-t", PAIR_X2, Y4 + top_pad, PAIR_W, title_h,
    "Streaming Platform", HDR_TITLE_FSZ, cid="plat")
txt("plat-sub", PAIR_X2, Y4 + top_pad + title_h + gap_ts, PAIR_W, sub_h,
    "Persistent (Kafka, Kinesis)", HDR_SUB_FSZ, color=CYAN[0])

# --- Consumer ---
rect("cons", BOX_X, Y5, BOX_W, BOX_H, *RED,
     bnd=[{"id": "cons-t", "type": "text"}])
txt("cons-t", BOX_X, Y5 + top_pad, BOX_W, title_h,
    "Consumers", HDR_TITLE_FSZ, cid="cons")
txt("cons-sub", BOX_X, Y5 + top_pad + title_h + gap_ts, BOX_W, sub_h,
    "Process or store data", HDR_SUB_FSZ, color=RED[0])

# --- Arrows ---
# Producer → Collector
arr("a1", BOX_X + BOX_W // 2, Y1 + BOX_H,
    [[0, 0], [0, ARR_GAP]], GREEN[0],
    sb={"elementId": "prod", "focus": 0, "gap": 4},
    eb={"elementId": "coll", "focus": 0, "gap": 4})

# Collector → Broker
arr("a2", BOX_X + BOX_W // 2, Y2 + BOX_H,
...
arr("a3", broker_cx, Y3 + BOX_H,
    [[0, 0], [0, ARR_GAP // 2], [mq_cx - broker_cx, ARR_GAP // 2], [mq_cx - broker_cx, ARR_GAP]],
    YELLOW[0],
...
arr("a4", broker_cx, Y3 + BOX_H,
    [[0, 0], [0, ARR_GAP // 2], [plat_cx - broker_cx, ARR_GAP // 2], [plat_cx - broker_cx, ARR_GAP]],
    YELLOW[0],
    sb={"elementId": "broker", "focus": 0, "gap": 4},
...
arr("a5", mq_cx, Y4 + BOX_H,
    [[0, 0], [0, ARR_GAP // 2], [cons_cx - mq_cx, ARR_GAP // 2], [cons_cx - mq_cx, ARR_GAP]],
    PURPLE[0],
    sb={"elementId": "mq", "focus": 0, "gap": 4},
    eb={"elementId": "cons", "focus": 0, "gap": 4})

# Streaming Platform → Consumer (fan-in right)
arr("a6", plat_cx, Y4 + BOX_H,
    [[0, 0], [0, ARR_GAP // 2], [cons_cx - plat_cx, ARR_GAP // 2], [cons_cx - plat_cx, ARR_GAP]],
    CYAN[0],
