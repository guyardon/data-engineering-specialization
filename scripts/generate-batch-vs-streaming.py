...
# Column headers
d.rect("h-batch", LEFT_X, HEADER_Y, COL_W, HEADER_H, BLUE[0], BLUE[1],
     bnd=[{"id": "ht-batch", "type": "text"}])
d.txt("ht-batch", LEFT_X, HEADER_Y, COL_W, HEADER_H,
    "Batch", 26, cid="h-batch")

d.rect("h-stream", RIGHT_X, HEADER_Y, COL_W, HEADER_H, RED[0], RED[1],
     bnd=[{"id": "ht-stream", "type": "text"}])
d.txt("ht-stream", RIGHT_X, HEADER_Y, COL_W, HEADER_H,
...
for i, (aspect, batch_txt, stream_txt) in enumerate(rows):
    ...
    d.txt(f"aspect-{i}", PAD_X, cur_y, CONTENT_W, aspect_h,
        aspect, 20, color=GRAY[0])

    card_y = cur_y + aspect_h + 8

    # Batch card
    d.rect(f"batch-{i}", LEFT_X, card_y, COL_W, rh, BLUE[0], BLUE[1], opacity=30,
         bnd=[{"id": f"bt-{i}", "type": "text"}])
    d.txt(f"bt-{i}", LEFT_X, card_y, COL_W, rh,
        batch_txt, 22, cid=f"batch-{i}")

    # Streaming card
    d.rect(f"stream-{i}", RIGHT_X, card_y, COL_W, rh, RED[0], RED[1], opacity=30,
         bnd=[{"id": f"st-{i}", "type": "text"}])
    d.txt(f"st-{i}", RIGHT_X, card_y, COL_W, rh,
        stream_txt, 22, cid=f"stream-{i}")

    cur_y = card_y + rh + ROW_GAP

# === WRITE FILE ===
outfile = "diagrams/batch-vs-streaming.excalidraw"
d.save(outfile)
