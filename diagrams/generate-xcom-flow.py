...
T1_X = PAD_X + 20
T1_Y = ROW1_Y + 30
rect("task1", T1_X, T1_Y, BW_TASK, BH, *BLUE,
...
XCOM_Y = ROW1_Y
rect("xcom", XCOM_X, XCOM_Y, BW_XCOM, BH_XCOM, *PURPLE,
     bnd=[{"id": "t_xcom", "type": "text"}])
txt("t_xcom", XCOM_X, XCOM_Y, BW_XCOM, BH_XCOM,
    "XCom Variable\n\nValue\nKey\nTimestamp\nDAG ID\nTask ID", 18, cid="xcom")

# Arrow: Task 1 → XCom Variable
A1_X = T1_X + BW_TASK
A1_Y = T1_Y + BH // 2
arr("a1", A1_X, A1_Y, [[0, 0], [GAP_H, 0]], BLUE[0],
...
rect("db", DB_X, DB_Y, DB_W, DB_H, *GRAY,
...
A2_Y = XCOM_Y + BH_XCOM
arr("a2", A2_X, A2_Y, [[0, 0], [0, 70]], PURPLE[0],
...
T2_Y = DB_Y
rect("task2", T2_X, T2_Y, BW_TASK, BH, *GREEN,
...
A3_X = DB_X + DB_W
A3_Y = DB_Y + DB_H // 2
arr("a3", A3_X, A3_Y, [[0, 0], [GAP_H, 0]], GRAY[0],
...
txt("sub", PAD_X, TITLE_Y + 38, CANVAS_W - 2 * PAD_X, 25,
    "Tasks exchange small data via the metadata database", 17, color=PURPLE[0])
