...
for lid, lx, title, subtitle, desc, color in layers:
    d.rect(f"{lid}_box", lx, BOX_Y, BOX_W, BOX_H, *color, fill="hachure", opacity=25,
...
for i, (lid, lx, *_) in enumerate(layers[:-1]):
    ...
    ax = lx + BOX_W
    ay = BOX_Y + BOX_H // 2
    d.arr(f"arr_{i}", ax, ay, [[0, 0], [GAP, 0]], layers[i][5][0],
...
STORAGE_H = 50
d.rect("storage", start_x - 10, STORAGE_Y, total_w + 20, STORAGE_H, *GRAY, fill="hachure", opacity=10, dashed=True,
     bnd=[{"id": "storage_t", "type": "text"}])
d.txt("storage_t", start_x - 10, STORAGE_Y, total_w + 20, STORAGE_H, "Object Storage (S3)", 20, color=GRAY[0], cid="storage")

# Dashed arrows from each box to storage
for lid, lx, *_ in layers:
    d.arr(f"store_{lid}", lx + BOX_W // 2, BOX_Y + BOX_H, [[0, 0], [0, 50]], GRAY[0], dash=True, op=40)
