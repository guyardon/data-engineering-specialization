...
txt("title", PAD_X, TITLE_Y, CONTENT_W, 40,
    "Replication vs Partitioning", 32)
txt("sub", PAD_X, TITLE_Y + 38, CONTENT_W, 25,
    "Two strategies for distributing data across nodes", 17, color=BLUE[0])
...
SRC_Y = 85
rect("src", SRC_X, SRC_Y, SRC_W, SRC_H, *GRAY,
...
# Column title
txt("rep_title", LEFT_X, SECTION_Y - 35, COL_W, 30,
    "Replication", 24, color=GREEN[0])
txt("rep_sub", LEFT_X, SECTION_Y - 10, COL_W, 20,
    "Same data on every node", 17, color=GREEN[0], op=70)
...
for i, label in enumerate(rep_nodes):
    ...
    nid = f"rep_n{i}"
    rect(nid, LEFT_X + INNER_PAD, ny, NODE_W, NODE_H, *GREEN,
...
arr("a_rep", SRC_X + SRC_W // 4, SRC_Y + SRC_H,
    [[0, 0], [LEFT_X + COL_W // 2 - SRC_X - SRC_W // 4, SECTION_Y + 20 - SRC_Y - SRC_H]],
    GREEN[0],
...
# Column title
txt("part_title", RIGHT_X, SECTION_Y - 35, COL_W, 30,
    "Partitioning", 24, color=PURPLE[0])
txt("part_sub", RIGHT_X, SECTION_Y - 10, COL_W, 20,
    "Disjoint subsets per node", 17, color=PURPLE[0], op=70)
...
for i, label in enumerate(part_nodes):
    ny = SECTION_Y + 20 + INNER_PAD + i * (NODE_H + NODE_GAP)
    nid = f"part_n{i}"
    rect(nid, RIGHT_X + INNER_PAD, ny, NODE_W, NODE_H, *PURPLE,
...
arr("a_part", SRC_X + 3 * SRC_W // 4, SRC_Y + SRC_H,
    [[0, 0], [RIGHT_X + COL_W // 2 - SRC_X - 3 * SRC_W // 4, SECTION_Y + 20 - SRC_Y - SRC_H]],
    PURPLE[0],
    sb={"elementId": "src", "focus": 0, "gap": 4},
