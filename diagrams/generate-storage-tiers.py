...
txt("title", PAD_X, TITLE_Y, CONTENT_W, 40,
    "Storage Tiers & AWS S3 Classes", 32)
txt("sub", PAD_X, TITLE_Y + 38, CONTENT_W, 25,
    "Trading access speed for cost", 17, color=BLUE[0])
...
def build_tier(tier_id, y, title, subtitle, color, s3_classes):
    ...
    txt(f"{tier_id}_title", PAD_X + INNER_PAD, y + INNER_PAD - 5,
        CONTENT_W - 2 * INNER_PAD, LABEL_H, title, 22, color=color[0])

    # Subtitle (Rule 14: subtitle color = stroke color)
    txt(f"{tier_id}_sub", PAD_X + INNER_PAD, y + INNER_PAD + LABEL_H - 8,
        CONTENT_W - 2 * INNER_PAD, 20, subtitle, 17, color=color[0], op=70)
...
hot_bottom = build_tier(
    "hot", Y,
    "Hot Storage", "Frequent access · SSD & memory · High cost",
    RED,
    ["S3 Express\nOne Zone", "S3 Standard"],
)

# Arrow hot → warm
arr("a_hw", PAD_X + CONTENT_W // 2, hot_bottom,
    [[0, 0], [0, ARROW_GAP]], GRAY[0],
    sb={"elementId": "hot", "focus": 0, "gap": 4},
    eb={"elementId": "warm", "focus": 0, "gap": 4})

# Warm tier
warm_bottom = build_tier(
    "warm", hot_bottom + ARROW_GAP,
    "Warm Storage", "Less frequent · Magnetic / hybrid disks · Medium cost",
    YELLOW,
    ["S3 Standard-IA", "S3 One Zone-IA"],
)

# Arrow warm → cold
arr("a_wc", PAD_X + CONTENT_W // 2, warm_bottom,
    [[0, 0], [0, ARROW_GAP]], GRAY[0],
...
cold_bottom = build_tier(
    "cold", warm_bottom + ARROW_GAP,
    "Cold Storage", "Infrequent / archive · Low-cost disks · High retrieval cost",
    BLUE,
    ["Glacier Instant\nRetrieval", "Glacier Flexible\nRetrieval", "Glacier Deep\nArchive"],
)
