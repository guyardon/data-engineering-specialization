...
# Title
txt("title", 0, TITLE_Y, CANVAS_W, 40,
    "Message Queue vs. Event Streaming", 32)

# --- MESSAGE QUEUE COLUMN (left) ---

# Pill
PILL_W = 180
rect("mq_pill", COL1_X + (BW - PILL_W) // 2, PILL_Y, PILL_W, PILL_H, *BLUE,
     bnd=[{"id": "mq_pill_t", "type": "text"}])
txt("mq_pill_t", COL1_X + (BW - PILL_W) // 2, PILL_Y, PILL_W, PILL_H,
    "Message Queue", 24, cid="mq_pill")

# Producer
rect("mq_prod", COL1_X, ROW1_Y, BW, BH, *GRAY,
     bnd=[{"id": "mq_prod_t", "type": "text"}])
txt("mq_prod_t", COL1_X, ROW1_Y, BW, BH, "Producer", 22, cid="mq_prod")

# Arrow: Producer -> Queue
arr("mq_a1", COL1_CX, ROW1_Y + BH, [[0, 0], [0, ARR_GAP]], GRAY[0],
    sb={"elementId": "mq_prod", "focus": 0, "gap": 4},
    eb={"elementId": "mq_queue", "focus": 0, "gap": 4})

# Message Queue (FIFO buffer)
rect("mq_queue", COL1_X, ROW2_Y, BW, BH, *BLUE,
     bnd=[{"id": "mq_queue_t", "type": "text"}])
txt("mq_queue_t", COL1_X, ROW2_Y, BW, BH, "Message Queue\n(FIFO buffer)", 20, cid="mq_queue")

# Arrow: Queue -> Consumer
arr("mq_a2", COL1_CX, ROW2_Y + BH, [[0, 0], [0, ARR_GAP]], BLUE[0],
    sb={"elementId": "mq_queue", "focus": 0, "gap": 4},
    eb={"elementId": "mq_cons", "focus": 0, "gap": 4})

# Label on last arrow: "message consumed & removed"
txt("mq_lab_consumed", COL1_CX + 12, ROW2_Y + BH + ARR_GAP // 2 - 14,
    160, 28, "message consumed\n& removed", 17, op=70)

# Consumer
rect("mq_cons", COL1_X, ROW3_Y, BW, BH, *PURPLE,
...
# Pill
ESP_PILL_W = 220
rect("es_pill", COL2_X + (BW - ESP_PILL_W) // 2, PILL_Y, ESP_PILL_W, PILL_H, *GREEN,
     bnd=[{"id": "es_pill_t", "type": "text"}])
txt("es_pill_t", COL2_X + (BW - ESP_PILL_W) // 2, PILL_Y, ESP_PILL_W, PILL_H,
    "Event Streaming", 24, cid="es_pill")

# Producer
rect("es_prod", COL2_X, ROW1_Y, BW, BH, *GRAY,
     bnd=[{"id": "es_prod_t", "type": "text"}])
txt("es_prod_t", COL2_X, ROW1_Y, BW, BH, "Producer", 22, cid="es_prod")

# Arrow: Producer -> Event Log
arr("es_a1", COL2_CX, ROW1_Y + BH, [[0, 0], [0, ARR_GAP]], GRAY[0],
    sb={"elementId": "es_prod", "focus": 0, "gap": 4},
    eb={"elementId": "es_log", "focus": 0, "gap": 4})

# Event Log (append-only)
rect("es_log", COL2_X, ROW2_Y, BW, BH, *GREEN,
     bnd=[{"id": "es_log_t", "type": "text"}])
txt("es_log_t", COL2_X, ROW2_Y, BW, BH, "Event Log\n(append-only)", 20, cid="es_log")

# Label near event log: "replay / reprocess"
txt("es_lab_replay", COL2_X + BW + 12, ROW2_Y + BH // 2 - 12,
    140, 24, "replay /\nreprocess", 17, op=70, align="left")

# Curved arrow looping back to the log (dashed, indicating replay)
arr("es_replay_arr", COL2_X + BW + 8, ROW2_Y + BH - 10,
    [[0, 0], [30, 30], [0, 50], [-20, 30]], GREEN[0], dash=True, op=50)

# Arrow: Event Log -> Consumer A (fan-out left)
FAN_A_CX = FAN_LEFT_X + CBW // 2
arr("es_a2a", COL2_CX, ROW2_Y + BH,
    [[0, 0], [0, ARR_GAP // 2], [FAN_A_CX - COL2_CX, ARR_GAP // 2], [FAN_A_CX - COL2_CX, ARR_GAP]],
    GREEN[0],
...
arr("es_a2b", COL2_CX, ROW2_Y + BH,
    [[0, 0], [0, ARR_GAP // 2], [FAN_B_CX - COL2_CX, ARR_GAP // 2], [FAN_B_CX - COL2_CX, ARR_GAP]],
    GREEN[0],
    sb={"elementId": "es_log", "focus": 0, "gap": 4},
    eb={"elementId": "es_cons_b", "focus": 0, "gap": 4})

# Consumer A
rect("es_cons_a", FAN_LEFT_X, ROW3_Y, CBW, CBH, *PURPLE,
     bnd=[{"id": "es_cons_a_t", "type": "text"}])
txt("es_cons_a_t", FAN_LEFT_X, ROW3_Y, CBW, CBH, "Consumer A", 22, cid="es_cons_a")

# Consumer B
rect("es_cons_b", FAN_RIGHT_X, ROW3_Y, CBW, CBH, *CYAN,
...
rect("divider", DIV_X, PILL_Y, 2, ROW3_Y + BH - PILL_Y, GRAY[0], "transparent",
