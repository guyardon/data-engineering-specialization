...
for i, (bid, title, desc, focus, color) in enumerate(approaches):
    ...
    d.rect(bid, x, y, COL_W, BOX_H, *color, bnd=[{"id": f"{bid}_t", "type": "text"}])
    ...
    d.txt(f"{bid}_t", x, y + top_pad, COL_W, title_h, title, 22, cid=bid)
    d.txt(
        f"{bid}_desc",
        x,
...
for i, (bid, title, desc, focus, color) in enumerate(approaches):
    ...
    d.txt(f"{fid}_t", x, FOCUS_Y, COL_W, FOCUS_H, focus, 16, cid=fid)
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/modeling-approaches"
