...
stages = [
    ...
    (
        "data",
        "2. Data",
        "Define requirements, label,\norganize — data engineer focus",
        BLUE,
        True,
    ),
    (
    ...
    (
        "deploy",
        "4. Deployment",
        "Productionize model, monitor,\nserve data for retraining",
        GREEN,
        True,
    ),
]
...
for i, (bid, title, desc, color, is_de) in enumerate(stages):
    ...
    if is_de:
        ...
        d.rect(
            ...
            PURPLE[0],
            PURPLE[1],
            bnd=[{"id": f"{bid}_badge_t", "type": "text"}],
        )
        d.txt(
            f"{bid}_badge_t",
            badge_x,
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/ml-lifecycle"
