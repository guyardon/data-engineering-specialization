...
from diagramlib import ExcalidrawDiagram, BLUE, GREEN, YELLOW, PURPLE, CYAN
...
# Section title
d.txt("sec1_title", 0, Y, CANVAS_W, 35, "1. Define Infrastructure", 28)
Y += 45
...
for j, (fname, desc) in enumerate(files_col1):
    ...
    d.txt(f"t_{fid}", INNER_X1, fy, INNER_W, BOX_H, f"{fname}\n{desc}", 18, cid=fid)
...
for j, (fname, desc) in enumerate(files_col2):
    ...
    d.txt(f"t_{fid}", INNER_X2, fy, INNER_W, BOX_H, f"{fname}\n{desc}", 18, cid=fid)
...
d.txt("sec2_title", 0, Y, CANVAS_W, 35, "2. Terraform CLI Workflow", 28)
Y += 45
...
d.txt("tf_label", PAD_X, Y + 8, CONTENT_W, 22, "Terraform Engine", 20, color=BLUE[0])
...
for i, (sid, cmd, desc, color) in enumerate(steps):
    ...
    d.txt(f"t_{sid}", sx, STEP_Y, STEP_W, STEP_H, cmd, 19, cid=sid)
    # Description below
    d.txt(f"d_{sid}", sx, STEP_Y + STEP_H + 5, STEP_W, 35, desc, 16, color=color[0])
...
for i, (label, color) in enumerate(props):
    ...
    d.txt(f"t_{pid}", px, PROP_Y, PROP_W, 36, label, 17, cid=pid)
...
d.txt("sec3_title", 0, Y, CANVAS_W, 35, "3. Cloud Infrastructure", 28)
Y += 45
...
for i, res in enumerate(resources):
    ...
    rid = f"r{i}"
    d.rect(
        rid, rx, RES_Y, RES_W, RES_H, *GREEN, bnd=[{"id": f"t_{rid}", "type": "text"}]
    )
    d.txt(f"t_{rid}", rx, RES_Y, RES_W, RES_H, res, 20, cid=rid)
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/terraform-workflow"
