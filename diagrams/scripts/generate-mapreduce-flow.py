...
from diagramlib import ExcalidrawDiagram, BLUE, GREEN, YELLOW, RED
...
for i, (bid, label, color) in enumerate(map_items):
    ...
    d.rect(
        bid, map_x, y, COL_W, PILL_H, *color, bnd=[{"id": f"{bid}_t", "type": "text"}]
    )
...
for i, (bid, label, color) in enumerate(shuf_items):
    ...
    d.rect(
        bid, shuf_x, y, COL_W, PILL_H, *color, bnd=[{"id": f"{bid}_t", "type": "text"}]
    )
...
for i, (bid, label, color) in enumerate(red_items):
    ...
    d.rect(
        bid, red_x, y, COL_W, PILL_H, *color, bnd=[{"id": f"{bid}_t", "type": "text"}]
    )
...
d.arr("a_map_shuf", map_x + COL_W, map_mid_y, [[0, 0], [COL_GAP, 0]], BLUE[0])

d.arr("a_shuf_red", shuf_x + COL_W, shuf_mid_y, [[0, 0], [COL_GAP, 0]], YELLOW[0])
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/mapreduce-flow"
