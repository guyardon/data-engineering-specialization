...
d.txt(
    "title", PAD_X, TITLE_Y, CONTENT_W, TITLE_H, "Inmon vs Kimball", 30, color="#1e1e1e"
)
...
for i, (bid, label, color) in enumerate(inmon_steps):
    h = BOX_H + 10 if "\n" in label else BOX_H
    d.rect(bid, LEFT_X, y, COL_W, h, *color, bnd=[{"id": f"{bid}_t", "type": "text"}])
...
for i, (bid, label, color) in enumerate(kimball_steps):
    h = BOX_H + 10 if "\n" in label else BOX_H
    d.rect(bid, RIGHT_X, y, COL_W, h, *color, bnd=[{"id": f"{bid}_t", "type": "text"}])
...
d.txt(
    ...
    COL_W,
    LABEL_H,
    "Quality first, extra modeling step",
    15,
    color=BLUE[0],
)
d.txt(
    ...
    COL_W,
    LABEL_H,
    "Speed first, direct star schemas",
    15,
    color=GREEN[0],
)
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/inmon-vs-kimball"
