...
for i, (bid, label, color) in enumerate(sources):
    ...
    d.rect(
        bid, x, SOURCES_Y, SRC_W, BH, *color, bnd=[{"id": f"{bid}_t", "type": "text"}]
    )
...
# === ROW 2: ETL Process ===
d.rect(
    "etl", PAD_X, ETL_Y, ETL_W, ETL_H, *YELLOW, bnd=[{"id": "etl_t", "type": "text"}]
)
d.txt("etl_t", PAD_X, ETL_Y, ETL_W, ETL_H, "ETL / ELT Process", 24, cid="etl")
...
d.txt(
    ...
    MODEL_W,
    INNER_H,
    "Modeled Tables (Star/Snowflake Schema)",
    20,
    cid="modeled",
)
...
# === WRITE FILE ===
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/warehouse-architecture"
