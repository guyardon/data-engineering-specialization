...
d.txt(
    "driver_sub",
    DRIVER_X,
    DRIVER_Y + driver_pad + driver_title_h + driver_gap,
    DRIVER_W,
    driver_sub_h,
...
d.rect("cm", CM_X, CM_Y, CM_W, CM_H, *PURPLE, bnd=[{"id": "cm_t", "type": "text"}])
d.txt("cm_t", CM_X, CM_Y, CM_W, CM_H, "Cluster Manager", 20, cid="cm")
CM_SUB_Y = CM_Y + CM_H + 5
CM_SUB_H = math.ceil(1 * 14 * 1.25)
d.txt(
    "cm_sub", CM_X, CM_SUB_Y, CM_W, CM_SUB_H, "YARN / Mesos / K8s", 14, color=PURPLE[0]
)
...
for i in range(3):
    ...
    d.txt(
        f"{wid}_label", wx, wl_y, WORKER_W, wl_h, f"Worker {i + 1}", 16, color=BLUE[0]
    )
    ...
    d.txt(
        f"ex{i}_t", EX_X, EX_Y + ex_pad, EX_W, ex_title_h, "Executor", 17, cid=f"ex{i}"
    )
    d.txt(
        f"ex{i}_sub",
        EX_X,
        EX_Y + ex_pad + ex_title_h + ex_gap,
        EX_W,
        ex_sub_h,
        "Tasks + Cache",
        13,
    ...
    d.arr(
        ...
        [
            [0, 0],
            [wx + WORKER_W // 2 - CM_X - CM_W // 2, WORKERS_Y - CM_SUB_Y - CM_SUB_H],
        ],
        PURPLE[0],
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/spark-architecture"
