...
d.txt(
    "t1_title", PAD_X, T1_Y + 10, CONTENT_W, 30, "Type 1 — Overwrite", 24, color=BLUE[0]
)
...
d.txt(
    "t1_hdr_t", TBL_X, BEFORE_Y, TBL_W, ROW_H, "key  |  name  |  city", 16, cid="t1_hdr"
)
...
d.txt(
    ...
    CONTENT_W,
    25,
    "No history preserved — previous value is lost",
    17,
    color=BLUE[0],
)
...
d.txt(
    ...
    TBL2_W,
    ROW_H,
    "key  |  name  |  city  |  effective  |  current",
    15,
    cid="t2_hdr",
)
...
d.txt(
    ...
    TBL2_W,
    ROW_H,
    "101  |  Alice  |  NYC  |  2024-01-01  |  false",
    15,
    cid="t2_row1",
    op=60,
)
...
d.txt(
    ...
    CONTENT_W,
    25,
    "Full history preserved — each version gets a new row",
    17,
    color=GREEN[0],
)

# === WRITE ===
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/scd-types"
