...
d.txt(
    "leader_t", LEADER_X, LEADER_Y, LEADER_W, LEADER_H, "Leader Node", 24, cid="leader"
)
...
d.txt("n1_label", N1_X, N1_LABEL_Y, NODE_W, N1_LABEL_H, "Node 1", 18, color=BLUE[0])
...
d.txt("n2_label", N2_X, N2_LABEL_Y, NODE_W, N2_LABEL_H, "Node 2", 18, color=BLUE[0])
...
d.arr(
    ...
    [
        [0, 0],
        [-(LEADER_X + LEADER_W // 2 - N1_X - NODE_W // 2), N1_Y - LEADER_Y - LEADER_H],
    ],
    YELLOW[0],
...
d.arr(
    ...
    [
        [0, 0],
        [N2_X + NODE_W // 2 - LEADER_X - LEADER_W // 2, N1_Y - LEADER_Y - LEADER_H],
    ],
    YELLOW[0],
    sb={"elementId": "leader", "focus": 0, "gap": 4},
...
d.rect("s3", S3_X, S3_BOX_Y, S3_W, S3_H, *GREEN, bnd=[{"id": "s3_t", "type": "text"}])
d.txt("s3_t", S3_X, S3_BOX_Y, S3_W, S3_H, "S3 (Columnar Storage)", 20, cid="s3")

# Arrows from nodes to S3
d.arr(
    "a_n1_s3",
    N1_X + NODE_W // 2,
    CN_CONTAINER_Y + CN_CONTAINER_H,
    [
        [0, 0],
        [
            S3_X + S3_W // 2 - N1_X - NODE_W // 2,
            S3_BOX_Y - CN_CONTAINER_Y - CN_CONTAINER_H,
        ],
    ],
    BLUE[0],
...
# === WRITE FILE ===
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/redshift-mpp"
