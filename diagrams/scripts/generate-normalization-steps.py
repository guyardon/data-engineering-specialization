...
from diagramlib import ExcalidrawDiagram, CYAN, GRAY, GREEN, RED, YELLOW
...
d.txt(
    ...
    CONTENT_W,
    TITLE_H,
    "Normalization: A Practical Example",
    30,
    color="#1e1e1e",
)
...
for i, (bid, title, color, desc) in enumerate(steps):
    d.rect(
        bid, PAD_X, y, BOX_W, BOX_H, *color, bnd=[{"id": f"{bid}_t", "type": "text"}]
    )
    ...
    d.txt(f"{bid}_t", PAD_X, y + top_pad, BOX_W, title_h, title, 24, cid=bid)
    d.txt(
        f"{bid}_sub",
        PAD_X,
        y + top_pad + title_h + gap,
        BOX_W,
        desc_h,
        desc,
        16,
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/normalization-steps"
