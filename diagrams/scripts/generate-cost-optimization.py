...
def dual_box(prefix, x, y, w, h, stroke, bg, title_text, sub_text, dashed=False):
    ...
    d.txt(title_id, x, title_y, w, title_h, title_text, 20, color="#1e1e1e", cid=box_id)
    d.txt(sub_id, x, sub_y, w, sub_h, sub_text, 14, color=stroke)
...
d.txt(
    "capex-opex-label", LEFT_X, y, WIDE_W, label_h, "CapEx vs OpEx", 20, color="#868e96"
)
y += label_h + 12
...
d.txt("toco-sub", WIDE_X, sub_y_t, WIDE_W, t_sub_h, toco_sub, 14, color=TOCO_STROKE)
...
for el in d.elements:
    if el["type"] in ("rectangle", "text", "line"):
        ...
        print(
            f"  {el['id']:25s}  type={el['type']:10s}  y={el['y']:.0f}  h={el.get('height', 0):.0f}  bottom={bottom:.0f}"
        )
...
# === WRITE FILE ===
outfile = "diagrams/artifacts/cost-optimization.excalidraw"
d.save(outfile)
print(f"\nWritten to: {outfile}")
