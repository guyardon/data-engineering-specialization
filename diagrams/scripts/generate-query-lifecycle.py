...
stages = [
    (
        "transport",
        "Transport Layer",
        "Client issues SQL query\nover network connection",
        GRAY,
    ),
    (
        "parser",
        "Query Parser",
        "Tokenize → syntax check →\naccess control → compile to bytecode",
        BLUE,
    ),
    (
        "optimizer",
        "Query Optimizer",
        "Evaluate candidate plans →\ncalculate cost → pick cheapest",
        YELLOW,
    ),
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/query-lifecycle"
