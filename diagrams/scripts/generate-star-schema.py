...
d.txt("title", PAD_X, TITLE_Y, CONTENT_W, TITLE_H, "Star Schema", 32, color="#1e1e1e")
...
d.txt("fact_t", CENTER_X, FACT_Y + ftp, FACT_W, ft_h, fact_title, 24, cid="fact")
d.txt(
    "fact_sub",
    CENTER_X,
    FACT_Y + ftp + ft_h + fg,
...
for bid, title, sub, color, dx, dy in dims:
    d.rect(bid, dx, dy, DIM_W, DIM_H, *color, bnd=[{"id": f"{bid}_t", "type": "text"}])
    ...
    d.txt(f"{bid}_t", dx, dy + dtp, DIM_W, dt_h, title, 22, cid=bid)
    d.txt(f"{bid}_sub", dx, dy + dtp + dt_h + dg, DIM_W, ds_h, sub, 15, color=color[0])
...
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/star-schema"
