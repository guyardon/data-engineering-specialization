...
from diagramlib import ExcalidrawDiagram, BLUE, YELLOW, PURPLE
...
for i, (bid, label, color) in enumerate(engines):
    ...
    d.rect(bid, x, QE_Y, QE_W, QE_H, *color, bnd=[{"id": f"{bid}_t", "type": "text"}])
...
d.txt(
    "otf_sub",
    PAD_X,
    OTF_Y + otf_top_pad + otf_title_h + otf_gap,
    CONTENT_W,
    otf_sub_h,
    otf_sub,
    17,
...
d.txt(
    "stor_sub",
    PAD_X,
    STOR_Y + stor_top_pad + stor_title_h + stor_gap,
    CONTENT_W,
    stor_sub_h,
    stor_sub,
    17,
...
# === WRITE FILE ===
name = sys.argv[1] if len(sys.argv) > 1 else "diagrams/artifacts/lakehouse-architecture"
