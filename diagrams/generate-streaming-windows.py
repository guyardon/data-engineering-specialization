...
d.txt("tum_label", PAD_X, TUM_Y + 10, LABEL_W, TUM_LABEL_H,
...
for i in range(3):
    ...
    bid = f"tw{i}"
    d.rect(bid, x, TUM_Y, tw, WIN_H, *BLUE,
         bnd=[{"id": f"{bid}_t", "type": "text"}])
    d.txt(f"{bid}_t", x, TUM_Y, tw, WIN_H,
        f"W{i+1}", 20, cid=bid)

# Time arrow below
TUM_ARROW_Y = TUM_Y + WIN_H + 12
d.txt("tum_time", VIS_X, TUM_ARROW_Y, VIS_W, math.ceil(1 * 15 * 1.25),
...
d.txt("slide_label", PAD_X, SLIDE_Y + 10, LABEL_W, SLIDE_LABEL_H,
...
for i in range(3):
    x = VIS_X + i * step
    bid = f"sw{i}"
    d.rect(bid, x, SLIDE_Y + i * 4, sw, WIN_H, *GREEN,
         opacity=70 if i > 0 else 100,
         bnd=[{"id": f"{bid}_t", "type": "text"}])
    d.txt(f"{bid}_t", x, SLIDE_Y + i * 4, sw, WIN_H,
        f"W{i+1}", 20, cid=bid, op=70 if i > 0 else 100)

SLIDE_NOTE_Y = SLIDE_Y + WIN_H + 20
d.txt("slide_time", VIS_X, SLIDE_NOTE_Y, VIS_W, math.ceil(1 * 15 * 1.25),
...
d.txt("sess_label", PAD_X, SESS_Y + 10, LABEL_W, SESS_LABEL_H,
...
d.rect("ss1", VIS_X, SESS_Y, s1w, WIN_H, *YELLOW,
...
d.txt("gap_label", gap_x, SESS_Y + 12, gap_w, math.ceil(1 * 16 * 1.25),
    "gap", 16, color=GRAY[0])

d.rect("ss2", VIS_X + s1w + gap_w, SESS_Y, s2w, WIN_H, *YELLOW,
     bnd=[{"id": "ss2_t", "type": "text"}])
d.txt("ss2_t", VIS_X + s1w + gap_w, SESS_Y, s2w, WIN_H, "Session 2", 20, cid="ss2")

SESS_NOTE_Y = SESS_Y + WIN_H + 12
d.txt("sess_time", VIS_X, SESS_NOTE_Y, VIS_W, math.ceil(1 * 15 * 1.25),
