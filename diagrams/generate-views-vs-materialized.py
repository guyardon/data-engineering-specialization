...
d.txt("hdr_view", PAD_X, HDR_Y, COL_W, HDR_H,
    "View (recomputes)", 20, color=YELLOW[0])
d.txt("hdr_mv", PAD_X + COL_W + COL_GAP, HDR_Y, COL_W, HDR_H,
...
for i, (bid, label, color) in enumerate(mv_steps):
    h = BOX_H + 10
    d.rect(bid, RX, y, COL_W, h, *color,
         bnd=[{"id": f"{bid}_t", "type": "text"}])
    d.txt(f"{bid}_t", RX, y, COL_W, h, label, 18, cid=bid)

    if i < len(mv_steps) - 1:
        d.arr(f"amv{i}", RX + COL_W // 2, y + h,
            [[0, 0], [0, ARROW_GAP]],
            color[0],
            sb={"elementId": bid, "focus": 0, "gap": 4},
...
d.txt("refresh", RX, REFRESH_Y, COL_W, REFRESH_H,
