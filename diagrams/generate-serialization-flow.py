...
B1_Y = 90
d.rect("b1", BOX_X, B1_Y, BW, BH, *BLUE, fill="hachure", bnd=[{"id": "t1", "type": "text"}])
...
d.rect("b2", BOX_X, B2_Y, BW, BH, *GREEN, fill="hachure", bnd=[{"id": "t2", "type": "text"}])
...
d.rect("b3", BOX_X, B3_Y, BW, BH, *PURPLE, fill="hachure", bnd=[{"id": "t3", "type": "text"}])
...
for i, (label, color) in enumerate(zip(fan_labels, fan_colors)):
    ...
    fid = f"fan{i}"
    ftid = f"fant{i}"
    d.rect(fid, fx, FAN_Y, SMALL_BW, SMALL_BH, *color, fill="hachure", bnd=[{"id": ftid, "type": "text"}])
