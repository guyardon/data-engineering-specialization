...
# Title
TITLE_Y = 15
txt("title", PAD_X, TITLE_Y, CONTENT_W, 40, "Serialization & Compression", 32)
txt(
    "sub",
    PAD_X,
    TITLE_Y + 38,
...
B1_Y = 90
rect("b1", BOX_X, B1_Y, BW, BH, *BLUE, bnd=[{"id": "t1", "type": "text"}])
...
rect("b2", BOX_X, B2_Y, BW, BH, *GREEN, bnd=[{"id": "t2", "type": "text"}])
...
rect("b3", BOX_X, B3_Y, BW, BH, *PURPLE, bnd=[{"id": "t3", "type": "text"}])
...
for i, (label, color) in enumerate(zip(fan_labels, fan_colors)):
    ...
    fid = f"fan{i}"
    ftid = f"fant{i}"
    rect(fid, fx, FAN_Y, SMALL_BW, SMALL_BH, *color, bnd=[{"id": ftid, "type": "text"}])
