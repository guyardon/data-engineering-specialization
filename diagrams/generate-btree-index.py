...
txt("title", PAD_X, TITLE_Y, CONTENT_W, TITLE_H,
    "B-Tree Index vs Sequential Scan", 30, color="#1e1e1e")
...
txt("seq_label", PAD_X, SEC_Y, LEFT_W, SEC_H,
    "Sequential Scan — O(n)", 20, color=RED[0])
txt("idx_label", RIGHT_X, SEC_Y, LEFT_W, SEC_H,
    "B-Tree Index — O(log n)", 20, color=GREEN[0])
...
for i, val in enumerate(rows):
    ...
    bid = f"row{i}"
    rect(bid, ROW_X, y, ROW_W, ROW_H, *color, opacity=opacity,
         bnd=[{"id": f"{bid}_t", "type": "text"}])
    txt(f"{bid}_t", ROW_X, y, ROW_W, ROW_H, val, 18, cid=bid, op=opacity)

    # Arrow showing scan direction (down the left side)
    if i < len(rows) - 1 and not is_target:
        arr(f"scan{i}", ROW_X - 12, y + ROW_H // 2,
            [[0, 0], [0, ROW_H + ROW_GAP]],
            RED[0], op=40)
...
txt("scan_count", PAD_X, SCAN_LABEL_Y, LEFT_W, SCAN_LABEL_H,
    "Checks 7 rows to find id=13", 16, color=RED[0])
...
rect("root", ROOT_X, ROOT_Y, NODE_W, NODE_H, *BLUE,
     bnd=[{"id": "root_t", "type": "text"}])
txt("root_t", ROOT_X, ROOT_Y, NODE_W, NODE_H, "[7 | 11]", 18, cid="root")
...
rect("int_l", L2_LEFT_X, L2_Y, NODE_W, NODE_H, *GRAY, opacity=50,
     bnd=[{"id": "int_l_t", "type": "text"}])
txt("int_l_t", L2_LEFT_X, L2_Y, NODE_W, NODE_H, "[1 | 3 | 5]", 16, cid="int_l", op=50)

# Right internal (highlighted path)
rect("int_r", L2_RIGHT_X, L2_Y, NODE_W, NODE_H, *BLUE,
     bnd=[{"id": "int_r_t", "type": "text"}])
txt("int_r_t", L2_RIGHT_X, L2_Y, NODE_W, NODE_H, "[11 | 13 | 15]", 16, cid="int_r")

# Arrows root -> children
arr("a_root_l", ROOT_X + NODE_W // 2, ROOT_Y + NODE_H,
    [[0, 0], [L2_LEFT_X + NODE_W // 2 - ROOT_X - NODE_W // 2, LEVEL_GAP]],
    GRAY[0], op=40,
    sb={"elementId": "root", "focus": 0, "gap": 4},
    eb={"elementId": "int_l", "focus": 0, "gap": 4})

arr("a_root_r", ROOT_X + NODE_W // 2, ROOT_Y + NODE_H,
...
rect("leaf", LEAF_X, L3_Y, NODE_W, NODE_H, *GREEN,
     bnd=[{"id": "leaf_t", "type": "text"}])
txt("leaf_t", LEAF_X, L3_Y, NODE_W, NODE_H, "id=13 ✓", 18, cid="leaf")

arr("a_int_leaf", L2_RIGHT_X + NODE_W // 2, L2_Y + NODE_H,
...
txt("idx_count", RIGHT_X, IDX_LABEL_Y, LEFT_W, IDX_LABEL_H,
    "3 steps to find id=13", 16, color=GREEN[0])

# === VERIFY ===
bottom = max(SCAN_LABEL_Y + SCAN_LABEL_H, IDX_LABEL_Y + IDX_LABEL_H) + 20
print(f"Canvas: {CANVAS_W}x{bottom}")

name = sys.argv[1] if len(sys.argv) > 1 else "btree-index"
