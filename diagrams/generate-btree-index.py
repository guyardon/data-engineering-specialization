...
d.txt("seq_label", PAD_X, SEC_Y, LEFT_W, SEC_H,
    "Sequential Scan — O(n)", 20, color=RED[0])
d.txt("idx_label", RIGHT_X, SEC_Y, LEFT_W, SEC_H,
...
for i, val in enumerate(rows):
    ...
    bid = f"row{i}"
    d.rect(bid, ROW_X, y, ROW_W, ROW_H, *color, opacity=opacity,
    ...
    if i < len(rows) - 1 and not is_target:
        d.arr(f"scan{i}", ROW_X - 12, y + ROW_H // 2,
            [[0, 0], [0, ROW_H + ROW_GAP]],
            RED[0], op=40)
...
d.txt("scan_count", PAD_X, SCAN_LABEL_Y, LEFT_W, SCAN_LABEL_H,
...
d.rect("root", ROOT_X, ROOT_Y, NODE_W, NODE_H, *BLUE,
...
d.rect("int_l", L2_LEFT_X, L2_Y, NODE_W, NODE_H, *GRAY, opacity=50,
...
d.rect("leaf", LEAF_X, L3_Y, NODE_W, NODE_H, *GREEN,
     bnd=[{"id": "leaf_t", "type": "text"}])
d.txt("leaf_t", LEAF_X, L3_Y, NODE_W, NODE_H, "id=13 ✓", 18, cid="leaf")

d.arr("a_int_leaf", L2_RIGHT_X + NODE_W // 2, L2_Y + NODE_H,
...
d.txt("idx_count", RIGHT_X, IDX_LABEL_Y, LEFT_W, IDX_LABEL_H,
