...
INSERT INTO fact_orders
    ...
    FROM orders;
`,T="1.13.0",o=`https://cdn.jsdelivr.net/npm/sql.js@${T}/dist/`;let i=null;function m(e){return new Promise((r,n)=>{const a=document.querySelector(`script[data-src="${e}"]`);if(a){if(a.dataset.loaded==="true"){r();return}a.addEventListener("load",()=>r()),a.addEventListener("error",()=>n(new Error(`Failed to load ${e}`)));return}const t=document.createElement("script");t.src=e,t.async=!0,t.dataset.src=e,t.addEventListener("load",()=>{t.dataset.loaded="true",r()}),t.addEventListener("error",()=>n(new Error(`Failed to load ${e}`))),document.head.appendChild(t)})}async function c(e){if(i)return i;i=(async()=>{if(e?.("Downloading SQLite engine (~1MB, one-time)..."),await m(`${o}sql-wasm.js`),!window.initSqlJs)throw new Error("sql.js failed to load");e?.("Initializing database...");const r=await window.initSqlJs({locateFile:a=>`${o}${a}`}),n=new r.Database;return n.exec(d),e?.("Ready"),n})();try{return await i}catch(r){throw i=null,r}}async function l(e,r){const n=await c(r);try{const t=n.exec(e).map(E=>({columns:E.columns,rows:E.values})),s=t.length===0?n.getRowsModified?.():void 0;return{tables:t,rowsAffected:s}}catch(a){return{tables:[],error:a instanceof Error?a.message:String(a)}}}export{c as initSQL,l as runSQL};
